...
func (t *Transformer) findAndInsertCtiSchema(ctx context, s map[string]any) (map[string]any, error) {
	...
	for _, cti := range ctis {
		for _, item := range ctx.history {
			if cti == item {
				...
				if val, ok := s[jsonschema.XCustomKey].(map[string]any); ok {
					newSchema[jsonschema.XCustomKey] = val
				}
	...
	if val, ok := s[jsonschema.XCustomKey].(map[string]any); ok {
		if val, ok := val[jsonschema.XAnnotationKey+metadata.Schema]; ok {
...
func (t *Transformer) getCtiSchema(ctx context, val any) (map[string]any, error) {
	...
	case string:
		...
		if val, ok := schema[jsonschema.XCustomKey].(map[string]any); ok {
			val[jsonschema.XAnnotationKey+metadata.Schema] = vv
		}
		return schema, nil
	case []any:
		...
		return map[string]any{"anyOf": schemas, jsonschema.XCustomKey: map[string]any{jsonschema.XAnnotationKey + metadata.Schema: vv}}, nil
	default:
