...
func shallowCopy(input map[string]any) map[string]any {
	...
	for key, value := range input {
		if key == jsonschema.XCustomKey {
			output[key] = jsonschema.DeepCopyMap(value.(map[string]any))
		} else {
			output[key] = value
		}
	}
...
func (t *Transformer) readMetadataCti(s map[string]any) ([]string, error) {
	// If schema has no custom metadata, return it as is.
	s, ok := s[jsonschema.XCustomKey].(map[string]any)
	if !ok {
		return nil, nil
	}
	// If schema has no reference to CTI schema, return it as is.
	val, ok := s[jsonschema.XAnnotationKey+metadata.Cti]
	if !ok {
		return nil, nil
	}
	switch v := val.(type) {
